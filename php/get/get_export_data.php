...
try {
        ...
        case 'feedback':
            $stmt = $conn->prepare("
                SELECT
                    ...
                    f.sqd0  AS 'SQD0', f.sqd1 AS 'SQD1',
                    f.sqd2  AS 'SQD2', f.sqd3 AS 'SQD3',
                    f.sqd4  AS 'SQD4', f.sqd5 AS 'SQD5',
                    f.sqd6  AS 'SQD6', f.sqd7 AS 'SQD7',
                    f.sqd8  AS 'SQD8',
            ...
            $headers = ['#','Department','Dept Code','Respondent Type','Sex','Age Group',
                        'Overall Rating','SQD0','SQD1','SQD2','SQD3','SQD4','SQD5',
                        'SQD6','SQD7','SQD8','Comment','Suggestions','Submitted At'];
            break;
            ...
            $stmt = $conn->prepare("
                SELECT
                    ...
                    ROUND(SUM(CASE WHEN f.rating >= 4 THEN 1 ELSE 0 END)
                          * 100.0 / NULLIF(COUNT(f.id),0), 1)                  AS 'Satisfaction Rate (%)',
                    ...
                    ROUND(AVG(f.sqd0),2) AS 'Avg SQD0',
                    ROUND(AVG(f.sqd1),2) AS 'Avg SQD1',
                    ROUND(AVG(f.sqd2),2) AS 'Avg SQD2',
                    ROUND(AVG(f.sqd3),2) AS 'Avg SQD3',
                    ROUND(AVG(f.sqd4),2) AS 'Avg SQD4',
                    ROUND(AVG(f.sqd5),2) AS 'Avg SQD5',
                    ROUND(AVG(f.sqd6),2) AS 'Avg SQD6',
                    ROUND(AVG(f.sqd7),2) AS 'Avg SQD7',
    ...
    // ── Output ──
    if ($format === 'excel') {
        // HTML table format — opens perfectly in Excel/WPS with proper columns
        ...
        // Helper: escape cell content for HTML
        $esc = fn($v) => htmlspecialchars((string)($v ?? ''), ENT_QUOTES, 'UTF-8');

        // Determine which columns are numeric (no text-format override needed)
        $numericCols = ['#','ID','Overall Rating','Total Responses','Total Feedback',
                        'Excellent (5)','Good (4)','Average (3)','Poor (2)','Very Poor (1)',
        ...
        <Styles>
          <Style ss:ID="header">
            ...
            <Interior ss:Color="#8B1A1A" ss:Pattern="Solid"/>
          ...
          <Style ss:ID="cellAlt">
            <Alignment ss:Vertical="Center" ss:WrapText="0"/>
            <Interior ss:Color="#FFF8F8" ss:Pattern="Solid"/>
            <Borders>
              <Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#EEEEEE"/>
            </Borders>
          </Style>
        </Styles>
        <Worksheet ss:Name="' . ucfirst($type) . '">
          <Table>
        ';

        // Column widths
        foreach ($headers as $h) {
            $w = in_array($h, $numericCols) ? 60 : (strlen($h) > 20 ? 160 : 110);
        ...
        foreach ($rows as $i => $row) {
            $style = ($i % 2 === 1) ? 'cellAlt' : 'cell';
            echo '<Row ss:Height="18">' . "\n";
            $vals = array_values($row);
            foreach ($vals as $ci => $val) {
                $hdr     = $headers[$ci] ?? '';
                $isNum   = in_array($hdr, $numericCols) && is_numeric($val);
                $type_   = $isNum ? 'Number' : 'String';
                $cellSt  = ($isNum || $type_ === 'Number') ? $style : 'cellText';
                echo '<Cell ss:StyleID="' . $cellSt . '"><Data ss:Type="' . $type_ . '">'
                   . $esc($val)
                   . '</Data></Cell>' . "\n";
            }
            echo '</Row>' . "\n";
        ...
        // CSV — proper comma-separated with UTF-8 BOM
